...
class DomainAgent:
    def __init__(self, domain: Domain, knowledge_base: IsolatedKnowledgeBase) -> None:
        self.domain = domain
    ...
    def answer(self, task_id: str, query: str, recipient: Domain) -> AgentMessage:
        ...
        if not records:
            return AgentMessage(
                message_id=str(uuid4()), task_id=task_id, sender=self.domain,
                recipient=recipient, purpose="domain_response", share_scope={recipient},
        ...
        return AgentMessage(
            message_id=str(uuid4()), task_id=task_id, sender=self.domain,
            recipient=recipient, purpose="domain_response",
            classification=record.classification, share_scope={recipient},
            conclusion={"status": "grounded", "summary": record.metadata.get("summary", record.text), **record.metadata.get("shareable", {})},
